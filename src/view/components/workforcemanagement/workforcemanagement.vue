<template>
  <div>
    <Header style="height:45px!important;line-height:35px!important;text-align:right;width:800px">
      <Button type="primary" @click="open" ><Icon type="md-add" />新增</Button>
    </Header>
      <Content>
      ...
      <Layout>
        <Content class="workcontent">
          <Form  label-position="left" :label-width="100" :model="formCustom"  ref="formCustom">
            ...
            <FormItem label="开始时间">
                <DatePicker type="time" placeholder="开始时间" style="width:100%"  v-model="formCustom[2].value" name='bco03'></DatePicker>
            </FormItem>
            <FormItem label="结束时间">
                <DatePicker type="time" placeholder="结束时间" style="width:100%"  v-model="formCustom[3].value" name='bco04'></DatePicker>
            </FormItem>
          </Form>
        </Content>
    </Layout>
...
export default {
  ...
  methods: {
    ...
    tabelGetList () {
      this.promiseShifts('/api/rateweb/cloud/SysSchedule/getShiftsList', 'get', { }).then((res) => {
        if (res.status === 200) {
          this.data1 = res.data
        }
      })
